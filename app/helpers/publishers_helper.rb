module PublishersHelper
  ...
  def publisher_overall_bat_balance(publisher)
    if overall_balance = publisher.wallet &&
       publisher.wallet.overall_balance.is_a?(Eyeshade::OverallBalance) &&
       publisher.wallet.overall_balance
  ...
  def publisher_converted_overall_balance(publisher)
    ...
    if overall_balance = publisher.wallet &&
       publisher.wallet.overall_balance.is_a?(Eyeshade::OverallBalance) &&
       publisher.wallet.overall_balance.amount_default_currency.present? &&
       publisher.wallet.overall_balance
       '%.2f' % overall_balance.amount_default_currency
  ...
  def publisher_last_settlement_balance(publisher)
    if balance = publisher.wallet &&
        publisher.wallet.last_settlement_balance.is_a?(Eyeshade::LastSettlementBalance) &&
        publisher.wallet.last_settlement_balance.amount_settlement_currency.present? &&
  ...
  def publisher_converted_last_settlement_balance(publisher)
    if balance = publisher.wallet &&
      ...
      settlement_currency = balance.settlement_currency
      return if settlement_currency == "BAT" || settlement_currency.blank?

      I18n.t("helpers.publisher.balance_pending_approximate", amount: balance.amount_settlement_currency, code: settlement_currency)
    end
  rescue => e
    require "sentry-raven"
    Raven.capture_exception(e)
  ...
  def publisher_last_settlement_date(publisher)
    if settlement_timestamp = publisher.wallet &&
        publisher.wallet.last_settlement_balance.is_a?(Eyeshade::LastSettlementBalance) &&
        publisher.wallet.last_settlement_balance.timestamp
